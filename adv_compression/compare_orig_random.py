...
def add_uniform_perturb(img, epsilon):
    perturbation = np.random.uniform(-epsilon, epsilon, img.shape)
    random_perturb_img = np.clip(0, 1, img + perturbation)
    random_perturb_img = random_perturb_img * 255  # back to 0 - 255 scale
    random_perturb_img = np.rint(random_perturb_img)
    random_perturb_img = random_perturb_img / 255

    return random_perturb_img


def compare_orig_random_compression(orig_img_dir, result_dir, compression_model, checkpoint_dir, epsilon, silent_mode=False, save_image=True):
    ...
    for f in img_files:
        ...
        # compression image with random perturbation
        perturb_img = add_uniform_perturb(orig_img, epsilon)
        adv_actual_bpp, adv_reconstruct_img, adv_eval_bpp_val, adv_mse_val, adv_psnr_val, adv_msssim_val, num_pixels_val = compressor.compress(perturb_img, orig_img)

        if save_image:
            save_path = result_dir + 'recon_random_' + f
            save_img(save_path, array_to_img(adv_reconstruct_img[0]))

        # compute distortions
        adv_mse = (np.square(orig_img * 255 - perturb_img * 255)).mean()
        ...
        compression_results[f] = {'perturb_mse': adv_mse, 'mse_increase': mse_increase, 'psnr_decrease': psnr_decrease,
                                  'orig_bpp': orig_actual_bpp, 'orig_eval_bpp': orig_eval_bpp_val, 'orig_mse': orig_mse_val,
        ...
        if not silent_mode:
            print(f'random adv mse: {adv_mse}')
...
if __name__ == '__main__':
    args = parse_args()
    compare_orig_random_compression(orig_img_dir=args.orig_img_dir,
